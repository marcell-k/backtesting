...
class Strategy(metaclass=ABCMeta):
    ...
    def __str__(self) -> str:
        params = ",".join(
            f"{i[0]}={i[1]}" for i in zip(self._params.keys(), map(_as_str, self._params.values()), strict=False)
        )
        if params:
            params = "(" + params + ")"
    ...
    def I(
        ...
        **kwargs,
        ...
        def _format_name(name: str) -> str:
            return name.format(
                *map(_as_str, args), **dict(zip(kwargs.keys(), map(_as_str, kwargs.values()), strict=False))
            )
...
class Order:
    ...
    def __init__(
        ...
        self.__open_sl: float | None = None
        self.__tag = tag
    ...
    def is_contingent(self):
        ...
        """
        return bool((parent := self.__parent_trade) and (self is parent._sl_order or self is parent._tp_order))
...
class Trade:
    ...
    def __init__(self, broker: _Broker, size: int, entry_price: float, entry_bar, tag) -> None:
        self.__broker = broker
        self.__size = size
    ...
    @property
    def open_sl(self) -> float | None:
        """
        The stop-loss price set at trade entry (i.e. the initial SL), as
        opposed to `Trade.sl`, which may have since been modified (e.g. by
        a trailing-stop strategy). `None` if no SL was set at entry.
        """
        return self.__open_sl
...
class _Broker:
        ...
        *,
        ...
        if is_long:
            if not (sl or -np.inf) < (limit or stop or adjusted_price) < (tp or np.inf):
                raise ValueError(
                    f"Long orders require: SL ({sl}) < LIMIT ({limit or stop or adjusted_price}) < TP ({tp})"
                )
        else:
            if not (tp or -np.inf) < (limit or stop or adjusted_price) < (sl or np.inf):
                raise ValueError(
                    f"Short orders require: TP ({tp}) < LIMIT ({limit or stop or adjusted_price}) < SL ({sl})"
                )
    ...
    def _process_orders(self):
        ...
        for order in list(self.orders):
            ...
            time_index = (
                (self._i - 1) if is_market_order and self._trade_on_close and not order.is_contingent else self._i
            )
            ...
            adjusted_price_plus_commission = adjusted_price + self._commission(order.size, price) / abs(order.size)
            ...
            if -1 < size < 1:
                size = copysign(
                    int((self.margin_available * self._leverage * abs(size)) // adjusted_price_plus_commission),
                    size,
                )
    ...
    def run(self, **kwargs) -> pd.Series:
        ...
        with np.errstate(invalid="ignore"):
            ...
            self._results = compute_stats(
                ...
                include_indicators=kwargs.pop("include_indicators", True),
            )
        ...
        def _optimize_grid() -> pd.Series | tuple[pd.Series, pd.Series]:
            ...
            with Pool() as pool, SharedMemoryManager() as smm:
                ...
                results = _tqdm(
                    pool.imap(
                        Backtest._mp_task,
                        ((bt, smm.df2shm(self._data), params_batch) for params_batch in _batch(param_combos)),
                    ),
            ...
            if pd.isnull(heatmap).all():
                ...
                stats = self.run(**dict(zip(heatmap.index.names, best_params, strict=False)))

            if return_heatmap:
                return stats, heatmap
            return stats

        def _optimize_sambo() -> pd.Series | tuple[pd.Series, pd.Series] | tuple[pd.Series, pd.Series, dict]:
            try:
                import sambo
            except ImportError:
            ...
            stats = self.run(**dict(zip(kwargs.keys(), res.x, strict=False)))
            output = [stats]

            if return_heatmap:
                heatmap = pd.Series(dict(zip(map(tuple, res.xv), -res.funv, strict=False)), name=maximize_key)
