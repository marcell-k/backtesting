...
def compute_stats(
    *, stats: pd.Series, data: pd.DataFrame, trades: pd.DataFrame = None, risk_free_rate: float = 0.0
) -> pd.Series:
    """
...
http://pandas.pydata.org/pandas-docs/stable/timeseries.html#offset-aliases
    ...
    if not isinstance(series, (pd.Series, pd.DataFrame)):
        assert isinstance(series, _Array), (
            "resample_apply(series=) must be `pd.Series`, `pd.DataFrame`, or a `Strategy.data.*` array"
        )
        series = series.s
...
class SignalStrategy(Strategy):
    ...
    def set_signal(
        self, entry_size: Sequence[float], exit_portion: Sequence[float] | None = None, *, plot: bool = True
    ):
        """
...
class TrailingStrategy(Strategy):
    ...
    def next(self):
        ...
        for trade in self.trades:
            if trade.is_long:
                trade.sl = max(trade.sl or -np.inf, self.data.Close[index] - self.__atr[index] * self.__n_atr)
            else:
                trade.sl = min(trade.sl or np.inf, self.data.Close[index] + self.__atr[index] * self.__n_atr)
    ...
    def run(self, **kwargs) -> pd.Series:
        ...
        for i, indicator in enumerate(strategy._indicators):
            if indicator._opts["overlay"]:
                strategy._indicators[i] = indicator.astype(np.float64, copy=True) / self._fractional_unit
...
class MultiBacktest:
    ...
    def run(self, **kwargs):
        ...
        with Pool() as pool, SharedMemoryManager() as smm:
            ...
            results = _tqdm(
                pool.imap(
                    self._mp_task_run, ((df_batch, self._strategy, self._bt_kwargs, kwargs) for df_batch in _batch(shm))
                ),
                total=len(shm),
...
__all__ = [
    ...
    if (
        (
            (callable(v) and getattr(v, "__module__", None) == __name__)  # callables from this module
            or k.isupper()
        )  # or CONSTANTS
