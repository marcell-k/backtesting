...
def compute_stats(
    ...
    if isinstance(trades, pd.DataFrame):
        ...
        n = len(trades)
        size = np.fromiter((t.size for t in trades), dtype=np.float64, count=n)
        entry_bar = np.fromiter((t.entry_bar for t in trades), dtype=np.int64, count=n)
        exit_bar = np.fromiter((t.exit_bar for t in trades), dtype=np.int64, count=n)
        entry_price = np.fromiter((t.entry_price for t in trades), dtype=np.float64, count=n)
        exit_price = np.fromiter((t.exit_price for t in trades), dtype=np.float64, count=n)
        open_sl = np.fromiter((t.open_sl for t in trades), dtype=np.float64, count=n)
        sl = np.fromiter((t.sl for t in trades), dtype=np.float64, count=n)
        tp = np.fromiter((t.tp for t in trades), dtype=np.float64, count=n)
        ...
        if include_indicators and len(trades_df) and strategy_instance:
    ...
    if is_datetime_index:
        ...
        s.loc["CAGR [%]"] = (
            ((s.loc["Equity Final [$]"] / equity[0]) ** (1 / time_in_years) - 1) * 100 if time_in_years else np.nan
        )
    ...
    s.loc["Alpha [%]"] = (
        s.loc["Return [%]"] - risk_free_rate * 100 - beta * (s.loc["Buy & Hold Return [%]"] - risk_free_rate * 100)
    )
    s.loc["Beta"] = beta
