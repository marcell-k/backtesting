...
class TestBacktest(TestCase):
    ...
    def test_assertions(self):
        class Assertive(Strategy):
            def init(self):
                ...
                assert try_(lambda: self.data.X, 1, AttributeError) == 1
                assert try_(lambda: self.data["X"], 1, KeyError) == 1
    ...
    def test_broker_params(self):
        bt = Backtest(GOOG.iloc[:100], SmaCross, cash=1000, spread=0.01, margin=0.1, trade_on_close=True)
        bt.run()
    ...
    def test_spread_commission(self):
        ...
        self.assertEqual(stats["_equity_curve"]["Equity"].iloc[2:4].round(2).tolist(), [9685.31, 9749.33])

        stats = Backtest(SHORT_DATA, S, cash=CASH, commission=(100, COMMISSION)).run()
        self.assertEqual(stats["_equity_curve"]["Equity"].iloc[2:4].round(2).tolist(), [9784.50, 9718.69])
        ...
        self.assertEqual(stats["_equity_curve"]["Equity"].iloc[2:4].round(2).tolist(), [9781.28, 9846.04])
    ...
    def test_commissions(self):
        ...
        EXPECTED_PAID_COMMISSION = (
            FIXED_COMMISSION + COMMISSION * SIZE * PRICE_ENTRY + FIXED_COMMISSION + COMMISSION * SIZE * PRICE_EXIT
        )
        ...
        self.assertEqual(stats["Equity Final [$]"], CASH + (PRICE_EXIT - PRICE_ENTRY) * SIZE - EXPECTED_PAID_COMMISSION)
    ...
    def test_compute_stats(self):
        ...
        self.assertSequenceEqual(
            sorted(stats["_equity_curve"].columns), sorted(["Equity", "DrawdownPct", "DrawdownDuration"])
        )

        self.assertEqual(len(stats["_trades"]), 66)

        indicator_columns = [
            f"{entry}_SMA(C,{n})" for entry in ("Entry", "Exit") for n in (SmaCross.fast, SmaCross.slow)
        ]
        self.assertSequenceEqual(
...
class TestOptimize(TestCase):
    ...
    def test_max_tries(self):
        ...
        for method, max_tries, random_state in (("grid", 5, 0), ("grid", 0.3, 0), ("sambo", 6, 0), ("sambo", 0.42, 0)):
            with self.subTest(method=method, max_tries=max_tries, random_state=random_state):
                _, heatmap = bt.optimize(
                    max_tries=max_tries, method=method, random_state=random_state, return_heatmap=True, **OPT_PARAMS
                )
...
class TestPlot(TestCase):
    ...
    def test_resample(self):
        ...
        with _tempfile() as f, patch(backtesting._plotting, "_MAX_CANDLES", 10), self.assertWarns(UserWarning):
    ...
    def test_indicator_name(self):
        ...
        class S(Strategy):
            def init(self):
                ...
                test_self.assertRaises(ValueError, self.I, _SMA, name=("SMA One", "SMA Two", "SMA Three"))
        ...
        with _tempfile() as f:
            bt.plot(
                filename=f, plot_drawdown=False, plot_equity=False, plot_pl=False, plot_volume=False, open_browser=False
            )
...
class TestLib(TestCase):
    ...
    def test_resample_apply(self):
        ...
        np.testing.assert_almost_equal(res.iloc[-48:].unique().tolist(), [1.242643, 1.242381, 1.242275], decimal=6)
    ...
    def test_compute_stats(self):
        ...
        long_stats = compute_stats(stats=stats, trades=only_long_trades, data=GOOG, risk_free_rate=0.02)
    ...
    def test_MultiBacktest(self):
        ...
        for start_method in mp.get_all_start_methods():
            with self.subTest(start_method=start_method), patch(backtesting, "Pool", mp.get_context(start_method).Pool):
