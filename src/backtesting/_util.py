...
class _Data:
    ...
    def pip(self) -> float:
        if self.__pip is None:
            self.__pip = float(
                10 ** -np.median([len(s.partition(".")[-1]) for s in self.__arrays["Close"].astype(str)])
            )
        return self.__pip
...
class SharedMemoryManager:
    """
    Simple shared memory contextmanager based on
    ...
    def df2shm(self, df):
        return tuple(
            ((column, *self.arr2shm(values)) for column, values in chain([(self._DF_INDEX_COL, df.index)], df.items()))
        )
