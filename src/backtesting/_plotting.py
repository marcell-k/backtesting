...
return this.labels[index] || "";
    ...
    def _plot_equity_section(is_return: bool = False) -> None:
        """Equity section."""
        # Max DD Dur. line
        ...
        fig = new_indicator_figure(y_axis_label=yaxis_label, **({"height": 80} if plot_drawdown else {"height": 100}))
        ...
        dd_timedelta_label = df["datetime"].iloc[round(dd_end)] - df["datetime"].iloc[dd_start]
        fig.line(
            [dd_start, dd_end],
    ...
    def _plot_drawdown_section() -> _figure:
        """Drawdown section."""
        ...
        fig.scatter(
            argmax, drawdown[argmax], legend_label=f"Peak (-{100 * drawdown[argmax]:.1f}%)", color="red", size=8
        )
        set_tooltips(fig, [("Drawdown", "@drawdown{-0.[0]%}")], renderers=[r])
        fig.yaxis.formatter = NumeralTickFormatter(format="-0.[0]%")
        return fig

    def _plot_pl_section() -> _figure:
        """Profit/Loss markers section."""
        fig = new_indicator_figure(y_axis_label="Profit / Loss", height=80)
        fig.add_layout(
            Span(
                location=0, dimension="width", line_color="#666666", line_dash="dashed", level="underlay", line_width=1
            )
        )
    ...
    def _plot_ohlc_trades() -> None:
        """Plot trade entry / exit markers on OHLC plot."""
        ...
        for i, value in enumerate(indicators):
            ...
            for j, arr in enumerate(value):
                ...
                if is_overlay:
                    ...
                    if is_scatter:
                        r = fig.circle(
                            "index", source_name, source=source, color=color, radius=BAR_WIDTH / 2 * 0.6, **kwargs
                        )
                    else:
...
def plot_heatmaps(
    ...
    for df in dfs:
        ...
        fig.rect(
            ...
            source=df,
            line_color=None,
            fill_color={"field": "_Value", "transform": cmap},
        )
        figs.append(fig)
